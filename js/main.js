class Calculator {
    constructor() {
        this.output = document.getElementById('output_display');
        this._sbrac = document.getElementById('_sbracket');
        this._ebrac = document.getElementById('_ebracket');
        this.input = document.querySelector('[name="input"]');
        this.bs = document.getElementById('bottom-sheet');
        this.flag = false;
    ...
    setInput(val) {
        ...
        if (val === 'DE' || val === 'Backspace') {
            if (this.display.input.length == 1) {
                ...
                this.display.input.pop();
                this.display.operation.pop();
                let sb = this.display.input.filter(ele => ele === '(').length;
                let eb = this.display.input.filter(ele => ele === ')').length;
                let count = sb - eb;
                if (count === 0) {
    ...
    readMemory() {
        var _a, _b;
        if ((_b = (_a = this.bs) === null || _a === void 0 ? void 0 : _a.firstElementChild) === null || _b === void 0 ? void 0 : _b.innerHTML) {
...
const toggle = document.getElementById('_2nd');
...
Array.from(child.children).forEach((ele) => {
    Array.from(ele.children).forEach((e) => {
        e.children[0].onclick = function (e) {
            e.stopPropagation();
            let val = e.target;
